...
def collisionDetect():
    ...
    # SEE TODO #1
    # - ian
    if(((playerX + width) > (block1X or block2X)) and ((playerX + width) < ((block1X + blockW) or (block2X + blockW))) and (playerY < (initY - blockH)) and ((playerX > (block1X or block2X))) and (playerX < ((block1X + blockW) or (block2X + blockW)))):
        pygame.quit()
        sys.exit()
    if(playerY <= 350):
        playerY = 350
